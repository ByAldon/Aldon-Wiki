...
import { PlusIcon, BookIcon, ChevronRightIcon, FolderPlusIcon } from './Icons';
...
interface SidebarProps {
  ...
  onCreateNewPage: (categoryId?: string) => void;
  onCreateNewCategory: (name: string) => void;
}

const Sidebar: React.FC<SidebarProps> = ({ pages, categories, activePageId, onSelectPage, onCreateNewPage, onCreateNewCategory }) => {
  ...
  return (
    <aside className="w-72 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col flex-shrink-0">
      ...
      <div className="p-2 flex space-x-2">
        <button
          onClick={() => onCreateNewPage(activePage?.categoryId)}
          className="w-full flex items-center justify-center px-3 py-2 text-sm font-medium text-white bg-primary rounded-md hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary dark:focus:ring-offset-gray-800"
        >
        ...
        <button
          ...
          className="flex-shrink-0 flex items-center justify-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 bg-gray-200 dark:bg-gray-700 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 focus:outline-none"
          title="New Category"
        >
